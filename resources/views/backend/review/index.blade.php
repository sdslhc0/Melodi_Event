<x-backend-layout>
    <x-slot:header>Review & Rating</x-slot:header>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        ...
        <div class="stat-card">
            ...
            <p class="text-3xl font-serif text-brown-50 mb-1">{{ $averageRating ? number_format($averageRating, 1) : '-' }}</p>
            <p class="text-brown-500 text-xs uppercase tracking-widest">Rata-rata Rating</p>
            {{-- Star display --}}
            <div class="flex gap-1 mt-2">
                @for($i = 1; $i <= 5; $i++)
                    <svg class="w-4 h-4 {{ $i <= round($averageRating ?? 0) ? 'text-yellow-400' : 'text-brown-700' }}" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                    </svg>
                @endfor
            </div>
        ...
        <div class="stat-card">
            ...
            <p class="text-3xl font-serif text-brown-50 mb-1">{{ $totalReviews }}</p>
            ...
            <div class="space-y-1.5">
                @for($i = 5; $i >= 1; $i--)
                    <div class="flex items-center gap-2">
                        <span class="text-[10px] text-brown-400 w-8">{{ $i }} ★</span>
                        <div class="flex-1 bg-brown-800/30 rounded-full h-2 overflow-hidden">
                            <div class="h-full rounded-full transition-all duration-500 {{ $i >= 4 ? 'bg-yellow-400' : ($i >= 3 ? 'bg-yellow-600' : 'bg-red-500') }}"
                                 style="width: {{ $totalReviews > 0 ? ($ratingCounts[$i] / $totalReviews) * 100 : 0 }}%"></div>
                        </div>
                        <span class="text-[10px] text-brown-500 w-6 text-right">{{ $ratingCounts[$i] }}</span>
                    </div>
                @endfor
            </div>
        </div>
    ...
    <div class="bg-dark-100 border border-brown-800/30 rounded-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-brown-800/30 flex items-center justify-between">
            ...
            <span class="text-brown-500 text-xs">{{ $totalReviews }} ulasan</span>
        </div>
                        ...
                        <tr class="hover:bg-dark-200/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($review->nama) }}&background=2e261f&color=c9a84c&bold=true&size=32" 
                                         alt="{{ $review->nama }}" class="w-8 h-8 rounded-full">
                                    <div>
                                        <div class="text-sm text-brown-200 font-medium">{{ $review->nama }}</div>
                                        <div class="text-xs text-brown-500">{{ $review->user->email ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-0.5">
                                    @for($i = 1; $i <= 5; $i++)
                                        <svg class="w-4 h-4 {{ $i <= $review->rating ? 'text-yellow-400' : 'text-brown-700' }}" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                                        </svg>
                                    @endfor
                                </div>
                                <span class="text-xs text-brown-500 mt-1 block">{{ $review->rating }}/5</span>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-sm text-brown-300 max-w-xs truncate" title="{{ $review->komentar }}">{{ $review->komentar }}</p>
                            </td>
                            <td class="px-6 py-4 text-sm text-brown-400">
                                {{ $review->created_at->format('d M Y') }}
                                <span class="block text-xs text-brown-600">{{ $review->created_at->format('H:i') }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <form method="POST" action="{{ route('backend.review.destroy', $review) }}" 
