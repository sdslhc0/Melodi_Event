...
<head>
    ...
    <title>{{ $title ?? 'MELODI - Wedding & Event Organizer' }}</title>
...
<body class="bg-dark text-brown-100 font-sans antialiased bg-pattern">
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 transition-all duration-300" x-data="{ mobileOpen: false, scrolled: false }"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
            <div class="flex justify-between items-center h-24 border-b border-brown-700/50">
                ...
                <div class="hidden md:flex items-center space-x-10 h-full">
                    <a href="{{ route('home') }}#about" class="text-brown-200 font-serif text-[11px] md:text-[13px] uppercase tracking-[0.1em] hover:text-gold-400 transition-colors duration-300">About</a>
                    
                    <div x-data="{ open: false }" class="relative h-full flex items-center" @mouseenter="open = true" @mouseleave="open = false">
                        <a href="{{ route('kategori') }}" class="font-serif text-[11px] md:text-[13px] uppercase tracking-[0.1em] transition-colors duration-300 {{ request()->routeIs('kategori') ? 'text-gold-500' : 'text-brown-200 hover:text-gold-400' }}">Categories</a>
                        <div x-show="open" 
                             ...
                             style="display: none;">
                            @if(isset($kategori_tipes))
                                @foreach($kategori_tipes as $tipe)
                                    <a href="{{ route('kategori') }}?tipe={{ urlencode($tipe) }}" 
                                       class="relative block px-6 py-4 text-center text-brown-200 font-serif text-[11px] md:text-xs uppercase tracking-[0.2em] border-b border-gold-500/10 last:border-b-0 hover:text-gold-400 hover:bg-gold-500/5 transition-all duration-300 group">
                                        ...
                                        <span class="relative z-10">{{ $tipe }}</span>
                    ...
                    <a href="{{ route('home') }}#contact" class="text-brown-200 font-serif text-[11px] md:text-[13px] uppercase tracking-[0.1em] hover:text-gold-400 transition-colors duration-300">Contact</a>
                    <a href="{{ route('home') }}#formulir-pemesanan" class="text-brown-200 font-serif text-[11px] md:text-[13px] uppercase tracking-[0.1em] hover:text-gold-400 transition-colors duration-300">Booking</a>
                    <a href="{{ route('home') }}#testimonial" class="text-brown-200 font-serif text-[11px] md:text-[13px] uppercase tracking-[0.1em] hover:text-gold-400 transition-colors duration-300">Review</a>

                    @guest
                        <a href="{{ route('login') }}" class="text-brown-200 font-serif text-[11px] md:text-[13px] uppercase tracking-[0.1em] hover:text-gold-400 transition-colors duration-300">Login</a>
                    @else
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" class="group flex items-center gap-3 focus:outline-none">
                                ...
                                <span class="text-gold-400 font-serif text-[11px] md:text-[13px] uppercase tracking-[0.1em] group-hover:text-gold-300 transition-colors duration-300">{{ Auth::user()->nama }}</span>
                            ...
                            <div x-show="open" @click.away="open = false" x-transition
                                ...
                                @if(Auth::user()->isAdmin())
                                    <a href="{{ route('backend.dashboard') }}" class="block px-4 py-2 text-sm text-brown-200 hover:text-gold-400 hover:bg-dark-200 transition-colors">Admin Panel</a>
                                @endif
                                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-brown-200 hover:text-gold-400 hover:bg-dark-200 transition-colors">Profil</a>
                                <a href="{{ route('booking.riwayat') }}" class="block px-4 py-2 text-sm text-brown-200 hover:text-gold-400 hover:bg-dark-200 transition-colors">Riwayat Booking</a>
                        ...
                        <div x-data="{ openProfile: false }" class="relative">
                            ...
                            <div x-show="openProfile" @click.away="openProfile = false" x-transition
                                ...
                                <div class="px-4 py-3 border-b border-gold-500/10 mb-1">
                                    <div class="text-brown-100 text-sm font-medium truncate">{{ Auth::user()->nama }}</div>
                                    <div class="text-brown-400 text-[10px] truncate">{{ Auth::user()->email }}</div>
                                </div>
                                @if(Auth::user()->isAdmin())
                                    <a href="{{ route('backend.dashboard') }}" class="block px-4 py-2.5 text-sm text-brown-200 hover:text-gold-400 hover:bg-gold-500/5 transition-colors">Admin Panel</a>
                                @endif
                                <a href="{{ route('profile.edit') }}" class="block px-4 py-2.5 text-sm text-brown-200 hover:text-gold-400 hover:bg-gold-500/5 transition-colors">Profil</a>
                                <a href="{{ route('booking.riwayat') }}" class="block px-4 py-2.5 text-sm text-brown-200 hover:text-gold-400 hover:bg-gold-500/5 transition-colors">Riwayat Booking</a>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
        ...
        <div x-show="mobileOpen" 
            ...
            <div class="px-6 py-2 flex flex-col">
                <a href="{{ route('home') }}#about" @click="mobileOpen = false" class="py-4 text-brown-200 font-serif text-[11px] uppercase tracking-[0.2em] hover:text-gold-400 transition-all duration-300" style="border-bottom: 1px solid rgba(201,168,76,0.1);">About</a>
                <a href="{{ route('kategori') }}" @click="mobileOpen = false" class="py-4 text-brown-200 font-serif text-[11px] uppercase tracking-[0.2em] hover:text-gold-400 transition-all duration-300" style="border-bottom: 1px solid rgba(201,168,76,0.1);">Categories</a>
                <a href="{{ route('home') }}#contact" @click="mobileOpen = false" class="py-4 text-brown-200 font-serif text-[11px] uppercase tracking-[0.2em] hover:text-gold-400 transition-all duration-300" style="border-bottom: 1px solid rgba(201,168,76,0.1);">Contact</a>
                <a href="{{ route('home') }}#formulir-pemesanan" @click="mobileOpen = false" class="py-4 text-brown-200 font-serif text-[11px] uppercase tracking-[0.2em] hover:text-gold-400 transition-all duration-300" style="border-bottom: 1px solid rgba(201,168,76,0.1);">Booking</a>
                <a href="{{ route('home') }}#testimonial" @click="mobileOpen = false" class="py-4 text-brown-200 font-serif text-[11px] uppercase tracking-[0.2em] hover:text-gold-400 transition-all duration-300" style="border-bottom: 1px solid rgba(201,168,76,0.1);">Review</a>
                
                @guest
                    <a href="{{ route('login') }}" class="py-4 text-gold-500 font-serif text-[11px] uppercase tracking-[0.2em] hover:text-gold-400 transition-all duration-300" style="border-bottom: 1px solid rgba(201,168,76,0.1);">Login</a>
                @endguest
            </div>
        </div>
    </nav>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
                    html: `<p class="text-[#a89880] text-[13px] font-sans leading-relaxed tracking-wide mt-2">{{ session('success') }}</p>`,
                    background: '#1a140e',
                    color: '#f0e8d8',
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    html: `<p class="text-[#a89880] text-[13px] font-sans leading-relaxed tracking-wide mt-2">{{ session('error') }}</p>`,
                    background: '#1a140e',
                    color: '#f0e8d8',
    ...
    <footer class="bg-dark pt-6 pb-6 mt-auto">
        {{-- Long Fading Line Top Border (No Dot) --}}
        ...
        <div class="max-w-4xl mx-auto px-4 flex flex-col items-center text-center">
            ...
            <div class="flex flex-wrap justify-center gap-5 md:gap-8 mb-6">
                 <a href="{{ route('home') }}" class="text-[#d4c8b0] text-[9px] md:text-[10px] uppercase tracking-[0.2em] relative group pb-1">
                 ...
                 <a href="{{ route('kategori') }}" class="text-[#d4c8b0] text-[9px] md:text-[10px] uppercase tracking-[0.2em] relative group pb-1">
                    <span class="relative z-10 transition-colors duration-300 group-hover:text-[#c9a84c]">Categories</span>
                    <span class="absolute bottom-0 left-1/2 w-0 h-[1px] bg-[#c9a84c] transition-all duration-300 group-hover:w-full group-hover:left-0 opacity-70"></span>
                 </a>
                 <a href="{{ route('home') }}#about" class="text-[#d4c8b0] text-[9px] md:text-[10px] uppercase tracking-[0.2em] relative group pb-1">
                    <span class="relative z-10 transition-colors duration-300 group-hover:text-[#c9a84c]">About</span>
                    <span class="absolute bottom-0 left-1/2 w-0 h-[1px] bg-[#c9a84c] transition-all duration-300 group-hover:w-full group-hover:left-0 opacity-70"></span>
                 </a>
                 <a href="{{ route('home') }}#contact" class="text-[#d4c8b0] text-[9px] md:text-[10px] uppercase tracking-[0.2em] relative group pb-1">
            ...
            <div class="text-[#7d7161] text-[9.5px] uppercase tracking-[0.1em] font-medium border-t border-[#4a4239]/40 pt-4 w-full max-w-xs mx-auto">
                &copy; {{ date('Y') }} MELODI Event. All rights reserved.
            </div>
            <br>
        </div>
    </footer>
