                ...
                <div x-data="{ showAllItems: false }">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        <?php $__currentLoopData = $acaras; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $acara): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="card-dark group h-full flex-col <?php echo e($loop->index < 5 ? 'flex' : ''); ?>" 
                                 x-data="{ showModal: false }"
                                 <?php if($loop->index >= 5 && $loop->index < 9): ?>
                    ...
                    <?php if($acaras->count() > 5): ?>
                        <div class="mt-12 flex flex-col items-center gap-4">
                            <button x-show="!showAllItems" @click="showAllItems = true" 
                                    class="px-8 py-3 border border-gold-500 text-gold-400 text-xs uppercase tracking-[0.2em] rounded-sm hover:bg-gold-500 hover:text-dark transition-all duration-300 font-medium w-full max-w-xs <?php echo e($acaras->count() <= 9 ? 'md:hidden block' : 'block'); ?>">
                                Lihat Semua
                            </button>
                            ...
                            <button x-show="showAllItems" style="display: none;" @click="showAllItems = false; window.scrollTo({ top: $el.closest('.grid').offsetTop - 100, behavior: 'smooth' })" 
                                    class="px-8 py-3 border border-brown-700/50 text-brown-300 text-xs uppercase tracking-[0.2em] rounded-sm hover:border-gold-500 hover:text-gold-400 transition-all duration-300 font-medium w-full max-w-xs <?php echo e($acaras->count() <= 9 ? 'md:hidden block' : 'block'); ?>">
                                Lihat Lebih Sedikit
                            </button>
                        </div>
                    <?php endif; ?>
...
<?php endif; ?>
<?php /**PATH C:\0704\2505\wedding-organizer\resources\views/frontend/kategori.blade.php ENDPATH**/ ?>