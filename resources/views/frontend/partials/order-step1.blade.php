...
<div x-show="step === 1" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
        ...
        @foreach($staticEventTypes as $tipe => $icon)
            <button type="button" 
                 @click="setJenisAcara('{{ $tipe }}'); if('{{ $tipe }}' !== 'Lainnya') customJenisAcara = ''"
                 :class="jenisAcara === '{{ $tipe }}' ? 'border-[#c9a84c] bg-[#c9a84c]/5' : 'border-[#4a4239] hover:border-[#6b5e4f]'"
                ...
                <div :class="jenisAcara === '{{ $tipe }}' ? 'text-[#c9a84c]' : 'text-[#a89880] group-hover:text-[#d4c8b0]'">
                ...
                <span class="text-[10px] uppercase tracking-[0.2em] font-serif font-bold"
                      :class="jenisAcara === '{{ $tipe }}' ? 'text-[#c9a84c]' : 'text-[#a89880] group-hover:text-[#d4c8b0]'">
                    {{ $tipe }}
                </span>
                {{-- Selected Indicator Dot --}}
                <div x-show="jenisAcara === '{{ $tipe }}'" class="w-1 h-1 bg-[#c9a84c] rounded-full mt-1"></div>
            </button>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <input type="date" name="tanggal" x-model="tanggalAcara" required min="{{ date('Y-m-d') }}" @change="checkDateAvailability()"
