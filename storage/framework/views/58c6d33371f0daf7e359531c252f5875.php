...
<body class="bg-dark text-brown-100 font-sans antialiased bg-pattern">
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 transition-all duration-300" x-data="{ mobileOpen: false, scrolled: false }"
            ...
            <div class="flex justify-between items-center h-24 border-b border-brown-700/50">
                ...
                <div class="flex items-center gap-4 md:hidden">
                    <?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(route('profile.edit')); ?>" class="w-8 h-8 rounded-full border border-gold-500/30 overflow-hidden bg-dark-200 flex items-center justify-center transition-all hover:border-gold-500/60">
                            <?php if(Auth::user()->foto): ?>
                                <img src="<?php echo e(asset('storage/' . Auth::user()->foto)); ?>" class="w-full h-full object-cover">
                            <?php else: ?>
                                <span class="text-gold-500 text-[10px] font-bold"><?php echo e(strtoupper(substr(Auth::user()->nama, 0, 1))); ?></span>
        ...
        <div x-show="mobileOpen" x-transition class="md:hidden bg-dark-100 border-t border-brown-800/30">
            <div class="px-4 py-4 space-y-3">
                <a href="<?php echo e(route('home')); ?>#about" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">About</a>
                <a href="<?php echo e(route('kategori')); ?>" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">Categories</a>
                <a href="<?php echo e(route('home')); ?>#contact" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">Contact</a>
                <a href="<?php echo e(route('home')); ?>#formulir-pemesanan" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">Booking</a>
                <a href="<?php echo e(route('home')); ?>#testimonial" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">Review</a>
                <?php if(auth()->guard()->check()): ?>
                    <div class="pt-4 pb-2 border-t border-brown-800/30 mt-4">
                        <div class="flex items-center mb-4">
                            <div class="w-10 h-10 rounded-full border border-gold-500/30 overflow-hidden bg-dark-200 flex items-center justify-center mr-3">
                                <?php if(Auth::user()->foto): ?>
                                    <img src="<?php echo e(asset('storage/' . Auth::user()->foto)); ?>" class="w-full h-full object-cover">
                                <?php else: ?>
                                    <span class="text-gold-500 text-xs font-bold"><?php echo e(strtoupper(substr(Auth::user()->nama, 0, 1))); ?></span>
                                <?php endif; ?>
                            </div>
                            <div>
                                <div class="text-brown-100 text-sm font-medium"><?php echo e(Auth::user()->nama); ?></div>
                                <div class="text-brown-400 text-[10px]"><?php echo e(Auth::user()->email); ?></div>
                            </div>
                        </div>
                        <div class="space-y-3">
                            <?php if(Auth::user()->isAdmin()): ?>
                                <a href="<?php echo e(route('backend.dashboard')); ?>" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">Admin Panel</a>
                            <?php endif; ?>
                            <a href="<?php echo e(route('profile.edit')); ?>" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">Profil</a>
                            <a href="<?php echo e(route('booking.riwayat')); ?>" class="block text-brown-200 text-sm hover:text-gold-400 transition-colors">Riwayat Booking</a>
                            <form method="POST" action="<?php echo e(route('logout')); ?>" class="block">
                                <?php echo csrf_field(); ?>
                                <button type="submit" class="text-brown-200 text-sm hover:text-gold-400 transition-colors text-left w-full">Logout</button>
                            </form>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="pt-4 border-t border-brown-800/30 mt-4">
                        <a href="<?php echo e(route('login')); ?>" class="block text-gold-500 font-medium text-sm hover:text-gold-400 transition-colors">Login</a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
<?php /**PATH C:\0704\1304\wedding-organizer\resources\views/layouts/frontend.blade.php ENDPATH**/ ?>