<x-frontend-layout>
    <x-slot:title>MELODI - Rayakan Momen Paling Berharga Hidupmu</x-slot:title>

    {{-- Hero Section --}}
    <section class="relative min-h-screen flex items-center overflow-hidden">
        {{-- Hero Flower Image --}}
        <div class="absolute right-0 md:right-12 top-0 h-full w-[100%] md:w-[60%] lg:w-[50%] z-0 pointer-events-none flex items-center justify-end overflow-visible" style="animation: fadeInRightFlower 1.5s ease-out 0.2s both;">
            <img src="{{ asset('bunga_pilihan.png') }}" alt="Bunga Pilihan" 
                 class="w-full h-[85vh] md:h-[110%] object-contain object-right flower-responsive"
                 style="mix-blend-mode: screen; filter: drop-shadow(0 0 15px rgba(201,168,76,0.15)) contrast(1.1) brightness(1.1);">
            
            <style>
                /* Pengaturan Posisi Bunga */
                .flower-responsive {
                    /* Khusus tampilan HP / Mobile */
                    transform: translate(30%, 15%) scale(0.8);
                    opacity: 0.25; /* Dibuat sedikit transparan agar teks tetap terbaca */
                }

                @media (min-width: 768px) {
                    .flower-responsive {
                        /* Khusus tampilan Laptop / PC (Sesuai yang Anda atur tadi) */
                        transform: translate(-20%, 4%) scale(1.10);
                        opacity: 0.9;
                    }
                }

                @keyframes fadeInRightFlower {
        ...
        <div class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-12 pt-16 md:pt-16 relative z-10 -mt-16 md:mt-0">
            <div class="max-w-3xl">
                ...
                <div class="flex flex-col sm:flex-row gap-3 md:gap-5" style="animation: fadeInUp 1s ease-out 0.3s both">
                    ...
                    <a href="{{ route('kategori') }}" class="inline-flex items-center justify-center w-full sm:w-auto text-center border border-[#4a4239] hover:border-[#e4c278] text-brown-200 hover:text-[#e4c278] bg-[#1f1a16]/50 px-8 md:px-12 py-4 font-serif text-xs uppercase tracking-[0.15em] font-bold transition-colors">Lihat Semua Layanan</a>
                </div>
            </div>
        </div>

        {{-- Decorative gold line (Removed since not in screenshot) --}}
    </section>
    ...
    <section id="paket-bundling" class="py-16 bg-dark">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Bundling Cards Grid --}}
            <div x-data="{ showAllBundling: false }">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @forelse($paketBundlings as $index => $bundling)
                    <div class="group relative bg-[#231d18] border border-[#4a4239]/60 overflow-hidden transition-all duration-500 hover:border-[#c9a84c]/50 hover:shadow-[0_8px_40_rgba(201,168,76,0.08)]"
                         x-show="showAllBundling || {{ $index }} < 3"
                         style="animation: fadeInUp 0.8s ease-out {{ ($index * 0.15) }}s both; {{ $index >= 3 ? 'display: none;' : '' }}">
                    ...
                    <div class="relative h-56 overflow-hidden">
                        ...
                        <div class="absolute top-4 right-4 bg-[#1a1715]/90 backdrop-blur-sm border border-[#c9a84c]/30 px-3 py-1.5">
                            <span class="text-[#c9a84c] font-serif text-sm font-bold">{{ $bundling->formatted_harga }}</span>
                        </div>
                    </div>

                    {{-- Content --}}
                    <div class="p-6">
                        <h3 class="font-serif text-xl text-[#f5f0e8] mb-2 group-hover:text-[#c9a84c] transition-colors duration-300">{{ $bundling->nama }}</h3>
                        
                        @if($bundling->acaras->count() > 0)
                        <div class="mb-3">
                            <ul class="space-y-1">
                        ...
                        @if($bundling->deskripsi)
                            <p class="text-[#a89880] text-[13px] leading-relaxed line-clamp-2 italic mb-1 opacity-80">"{{ $bundling->deskripsi }}"</p>
                        @endif
                        ...
                        <div class="mt-5 pt-5 border-t border-[#4a4239]/40 flex items-center justify-between">
                            ...
                            <a href="{{ url('/?paket_bundling=' . $bundling->id . '#formulir-pemesanan') }}" 
                               onclick="confirmBooking(event, this.href, '{{ addslashes($bundling->nama) }}')" 
                ...
                @if($paketBundlings->count() > 3)
                    <div class="mt-12 flex flex-col items-center gap-4">
                        <button x-show="!showAllBundling" @click="showAllBundling = true" 
                                class="px-8 py-3 border border-[#c9a84c] text-[#c9a84c] text-xs uppercase tracking-[0.2em] rounded-sm hover:bg-[#c9a84c] hover:text-[#1a140e] transition-all duration-300 font-medium w-full max-w-xs block">
                            Lihat Semua
                        </button>
                        
                        <button x-show="showAllBundling" style="display: none;" @click="showAllBundling = false; window.scrollTo({ top: $el.closest('#paket-bundling').offsetTop - 50, behavior: 'smooth' })" 
                                class="px-8 py-3 border border-[#4a4239]/50 text-[#a89880] text-xs uppercase tracking-[0.2em] rounded-sm hover:border-[#c9a84c] hover:text-[#c9a84c] transition-all duration-300 font-medium w-full max-w-xs block">
                            Lihat Lebih Sedikit
                        </button>
                    </div>
                @endif
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                ...
                <div>
                    ...
                    {{-- Google Maps Embed --}}
                    {{-- Ganti src iframe di bawah dengan link Google Maps embed setelah Anda memberi titik koordinat --}}
                ...
                <div class="contact-form-wrap bg-dark-100">
                    ...
                    <form action="{{ route('contact.send') }}" method="POST" class="space-y-5 mt-8">
                        @csrf
                        <div>
    ...
    {{-- Formulir Pemesanan Section --}}
    @include('frontend.partials.order-form')
            ...
            <div class="flex overflow-x-auto snap-x snap-mandatory hide-scroll-bar gap-6 pb-8" style="scrollbar-width: none; -ms-overflow-style: none;">
                ...
                <div class="w-80 lg:w-96 flex-none snap-center shrink-0 border border-[#4a4239]/60 bg-[#231d18] p-6 lg:p-8 transition-colors hover:border-[#c9a84c]/50">
                    ...
                    <div class="flex gap-1 mb-5">
                        @for($i=1; $i<=5; $i++)
                        <svg class="w-4 h-4 {{ $i <= $review->rating ? 'text-[#c9a84c]' : 'text-[#4a4239]' }}" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center gap-3 mt-auto border-t border-[#4a4239]/40 pt-4">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($review->nama) }}&background=2e261f&color=c9a84c&bold=true" alt="{{ $review->nama }}" class="w-10 h-10 rounded-full border border-[#4a4239]">
                        <div>
                            <span class="text-[#d4c8b0] font-serif text-[13px] font-medium block">{{ $review->nama }}</span>
                            <span class="text-[#6b5e4f] text-[10px]">{{ $review->created_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
                @endforeach

                {{-- Static testimonials (always shown as fallback/supplement) --}}
                @foreach($staticTestimonials as $testi)
                <div class="w-80 lg:w-96 flex-none snap-center shrink-0 border border-[#4a4239]/60 bg-[#231d18] p-6 lg:p-8 transition-colors hover:border-[#c9a84c]/50">
                    {{-- Stars --}}
                    <div class="flex gap-1 mb-5">
                        @for($i=1; $i<=5; $i++)
                        <svg class="w-4 h-4 {{ $i <= $testi['rating'] ? 'text-[#c9a84c]' : 'text-[#4a4239]' }}" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                        </svg>
                        @endfor
                    </div>
                    ...
                    {{-- Profile --}}
                    <div class="flex items-center gap-3 mt-auto border-t border-[#4a4239]/40 pt-4">
                        <img src="{{ $testi['image'] }}" alt="{{ $testi['name'] }}" class="w-10 h-10 rounded-full border border-[#4a4239]">
                        <span class="text-[#d4c8b0] font-serif text-[13px] font-medium">{{ $testi['name'] }}</span>
                    </div>
                </div>
                @endforeach
            </div>
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <button type="button"
                                            ...
                                            <svg class="w-10 h-10 transition-colors duration-200"
                                                 :class="(hoverRating >= {{ $i }} || rating >= {{ $i }}) ? 'text-[#c9a84c] drop-shadow-[0_0_6px_rgba(201,168,76,0.4)]' : 'text-[#4a4239]'"
