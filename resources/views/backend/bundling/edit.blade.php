<x-backend-layout>
    <x-slot:header>Edit Paket Bundling</x-slot:header>

    <div class="max-w-3xl bg-dark-100 border border-brown-800/30 rounded-xl p-6 md:p-8">
        <form action="{{ route('backend.bundling.update', $bundling) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $bundling->nama) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="harga" value="{{ old('harga', $bundling->harga) }}" required
                           class="w-full bg-dark border border-brown-800 rounded-lg px-4 py-2.5 text-brown-100 focus:border-gold-500 focus:ring focus:ring-gold-500/20 outline-none transition-all">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3"
                              class="w-full bg-dark border border-brown-800 rounded-lg px-4 py-2.5 text-brown-100 focus:border-gold-500 focus:ring focus:ring-gold-500/20 outline-none transition-all resize-none">{{ old('deskripsi', $bundling->deskripsi) }}</textarea>
                                    ...
                                    @foreach($acaras as $acara)
                                        <label class="flex items-center gap-3 p-3 rounded-lg border border-brown-800/30 hover:border-gold-500/40 transition-colors cursor-pointer group">
                                            <input type="checkbox" name="layanan[]" value="{{ $acara->id }}"
                                                   {{ in_array($acara->id, old('layanan', $selectedAcaraIds)) ? 'checked' : '' }}
            ...
            <div class="mt-8 flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 bg-gold-500 text-dark-900 font-medium rounded-lg hover:bg-gold-400 transition-colors">Simpan Perubahan</button>
            </div>
        </form>
    </div>
